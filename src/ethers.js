...
(function (global, factory) {
  ...
  class FetchResponse {
    ...
    assertOk() {
      ...
      let { message: message, error: error } = this.#error;
      if (message === '') {
  ...
  const crypto$1 =
    typeof globalThis === 'object' && 'crypto' in globalThis
      ? globalThis.crypto
      : undefined;
  /*! noble-hashes - MIT License (c) 2022 Paul Miller (paulmillr.com) */ const u8a$1 =
  ...
  const bitSet = (n, pos, value) =>
    n | ((value ? _1n$4 : _0n$3) << BigInt(pos));
  ...
  /*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */ const _0n$2 =
      BigInt(0),
    _1n$3 = BigInt(1),
    _2n$1 = BigInt(2),
    _3n$1 = BigInt(3);
  ...
  /*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */ const _0n$1 =
    BigInt(0);
  ...
  /*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */ function validatePointOpts(
    curve,
  ) {
  ...
  class Signature {
    ...
    static from(sig) {
      ...
      const { networkV: networkV, v: v } = (function (
        _v,
        yParityAndS,
        yParity,
      ) {
  ...
  function read_trie(next) {
    ...
    function expand({ S: S, B: B }, cps, saved) {
  ...
  function init() {
    ...
    for (let { V: V, M: M } of new Set(WHOLE_MAP.values())) {
      let recs = [];
      for (let cp of V) {
        let gs = GROUPS.filter((g) => group_has_cp(g, cp));
        let rec = recs.find(({ G: G }) => gs.some((g) => G.has(g)));
        if (!rec) {
          rec = { G: new Set(), V: [] };
          recs.push(rec);
        }
  ...
  function flatten(split) {
    return split
      .map(({ input: input, error: error, output: output }) => {
        if (error) {
    ...
    constructor(_types) {
      ...
      Object.keys(_types).forEach((type) => {
        types[type] = _types[type].map(({ name: name, type: type }) => {
          let { base: base, index: index } = splitArray(type);
  ...
  const BuiltinErrors = {
    '0x08c379a0': {
      ...
      inputs: ['string'],
      reason: (message) =>
        `reverted with reason string ${JSON.stringify(message)}`,
    },
    '0x4e487b71': {
  ...
  class TransactionReceipt {
    ...
    constructor(tx, provider) {
      this.#logs = Object.freeze(tx.logs.map((log) => new Log(log, provider)));
      let gasPrice = BN_0$2;
  ...
  class TransactionResponse {
    ...
    async confirmations() {
      if (this.blockNumber == null) {
        const { tx: tx, blockNumber: blockNumber } = await resolveProperties({
    ...
    async wait(_confirms, _timeout) {
      ...
      const checkReplacement = async () => {
        ...
        const { blockNumber: blockNumber, nonce: nonce } =
  ...
  async function resolveArgs(_runner, inputs, args) {
    ...
    return await Promise.all(
      inputs.map((param, index) =>
        param.walkAsync(args[index], (type, value) => {
  ...
  function buildWrappedFallback(contract) {
    ...
    const method = async (overrides) => await send(overrides);
  ...
  async function hasSub(contract, event) {
    const { subs: subs } = getInternal(contract);
  ...
  async function getSub(contract, operation, event) {
    ...
    const { addr: addr, subs: subs } = getInternal(contract);
      ...
      const listener = (log) => {
        ...
        if (foundFragment) {
          ...
          emit(
            ...
            [],
            (listener) =>
              new ContractUnknownEventPayload(contract, listener, event, log),
          );
        }
      };
      let starting = [];
  ...
  class BaseContract {
    ...
    async queryFilter(event, fromBlock, toBlock) {
      ...
      const { addr: addr, addrPromise: addrPromise } = getInternal(this);
      const address = addr ? addr : await addrPromise;
      const { fragment: fragment, topics: topics } = await getSubInfo(
        this,
        event,
      );
      const filter = {
    ...
    async listenerCount(event) {
      ...
      const { subs: subs } = getInternal(this);
      let total = 0;
      for (const { listeners: listeners } of subs.values()) {
  ...
  class AbstractProvider {
    ...
    _getFilter(filter) {
      ...
      if (
        ...
        return Promise.all([Promise.all(address), fromBlock, toBlock]).then(
          (result) => resolve(result[0], result[1], result[2]),
        );
      }
    ...
    async #checkNetwork(promise) {
      const { value: value } = await resolveProperties({
    ...
    async call(_tx) {
      const { tx: tx, blockTag: blockTag } = await resolveProperties({
    ...
    async getBlock(block, prefetchTxs) {
      const { network: network, params: params } = await resolveProperties({
        network: this.getNetwork(),
    ...
    async getTransactionResult(hash) {
      const { result: result } = await resolveProperties({
        network: this.getNetwork(),
    ...
    async listenerCount(event) {
      ...
      let total = 0;
      for (const { listeners: listeners } of this.#subs.values()) {
        total += listeners.length;
      }
      return total;
    }
    ...
    async removeAllListeners(event) {
      if (event) {
        ...
        for (const [tag, { started: started, subscriber: subscriber }] of this
          .#subs) {
          if (started) {
            subscriber.stop();
          }
  ...
  class JsonRpcApiProvider extends AbstractProvider {
        ...
        while (payloads.length) {
          ...
          (async () => {
            ...
            try {
              ...
              for (const { reject: reject } of batch) {
                reject(error);
              }
            }
          })();
    ...
    _start() {
      ...
      this.#notReady = null;
        ...
        try {
            ...
            console.log(
            'JsonRpcProvider failed to detect network and cannot start up (perhaps the URL is wrong or the node is not started)',
            );
            this.emit(
            'error',
    ...
    async getSigner(address) {
      ...
      const { accounts: accounts } = await resolveProperties({
        network: this.getNetwork(),
    ...
    destroy() {
      ...
      for (const { payload: payload, reject: reject } of this.#payloads) {
        reject(
          makeError(
            'provider destroyed; cancelled request',
            'UNSUPPORTED_OPERATION',
  ...
  class AlchemyProvider extends JsonRpcProvider {
    ...
    async _perform(req) {
      if (req.method === 'getTransactionResult') {
        const { trace: trace, tx: tx } = await resolveProperties({
      ...
      switch (req.method) {
        ...
        case 'broadcastTransaction':
          return this.fetch(
            ...
            this._checkError(req, error, req.signedTransaction),
          );
        case 'getBlock':
          if ('blockTag' in req) {
            return this.fetch('proxy', {
  ...
  class WebSocketProvider extends SocketProvider {
    ...
    constructor(url, network, options) {
      super(network, options);
      if (typeof url === 'string') {
        this.#connect = () => new _WebSocket(url);
  ...
  function checkQuorum(quorum, results) {
    const tally = new Map();
    for (const { value: value, tag: tag, weight: weight } of results) {
  ...
  function getFuzzyMode(quorum, results) {
    ...
    for (const { weight: weight, value: value } of results) {
    ...
    for (const { weight: weight, result: result } of tally.values()) {
      if (
        weight >= quorum &&
        (weight > bestWeight ||
  ...
  class FallbackProvider extends AbstractProvider {
    ...
    async #checkQuorum(running, req) {
      ...
      for (const runner of running) {
        if (runner.result != null) {
          const { tag: tag, value: value } = normalizeResult(runner.result);
          results.push({
            tag: tag,
            value: value,
    ...
    async _perform(req) {
      if (req.method === 'broadcastTransaction') {
        ...
        const broadcasts = this.#configs.map(
          async ({ provider: provider, weight: weight }, index) => {
            try {
    ...
    async destroy() {
      for (const { provider: provider } of this.#configs) {
        provider.destroy();
      }
      super.destroy();
    }
  ...
  class BaseWallet extends AbstractSigner {
    ...
    async signTransaction(tx) {
      const { to: to, from: from } = await resolveProperties({
  ...
  /*! MIT License. Copyright 2015-2022 Richard Moore <me@ricmoo.com>. See LICENSE.txt. */ var __classPrivateFieldGet$2 =
  ...
  function decryptKeystoreJsonSync(json, _password) {
    ...
    const { salt: salt, N: N, r: r, p: p, dkLen: dkLen } = params;
